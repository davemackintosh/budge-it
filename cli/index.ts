...
import bankIndexes from "@bank-list";
...
const skipLines = process.argv[3] ? Number(process.argv[3]) : 0;
...
new Promise<string>((resolve, reject) =>
  ...
  .then((probableEntries: string[][]) =>
    probableEntries.filter((entry: string[]) => entry.length > 0)
  )
  .then((entries: string[][]) => {
    if (indexes.balance) {
      entries.unshift([
  ...
  .then((parsedEntries: ParsedEntry[]) => {
    loggers.forEach((logger: PostProcessor) =>
